...
import { Settings, ToggleLeft, DollarSign, Users, BarChart3, Shield, Brain, Cpu } from 'lucide-react';
import { DEFAULT_FEATURE_CONFIG, SUBSCRIPTION_TIERS } from '../config/adminConfig';
import { isPlatformAdmin } from '../../auth/config/roles';

const AdminSettings = ({ currentConfig = DEFAULT_FEATURE_CONFIG, onConfigChange, userData }) => {
  const [config, setConfig] = useState(currentConfig);
  const [activeTab, setActiveTab] = useState('clinical');
  const [showPlatformConfig, setShowPlatformConfig] = useState(false);

  // Secret access method for emergency platform configuration access
  useEffect(() => {
    let keys = [];
    ...
    const handleKeyDown = (e) => {
      ...
      if (keys.join(',') === secretCode.join(',')) {
        ...
        console.log("Platform configuration unlocked");
      }
    };
  ...
  const getVisibleTabs = () => {
    ...
    // Only show Platform Configuration tab for Platform Administrator or with secret access
    if (isPlatformAdmin(userData) || showPlatformConfig) {
      baseTabs.push({ 
        id: 'ai', 
        label: 'Platform Configuration', // More discrete name
        icon: Cpu 
      });
    }
      ...
      <div className="space-y-4">
        ...
        {activeTab === 'ai' && (
          <div className="space-y-6">
            <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4 border border-orange-200 dark:border-orange-700 mb-6">
              <h3 className="text-lg font-semibold text-orange-800 dark:text-orange-200 mb-2 flex items-center">
                <Shield className="w-5 h-5 mr-2" />
                Platform Administrator Access
              </h3>
              <p className="text-sm text-orange-700 dark:text-orange-300">
                This section contains advanced platform configuration options. Changes here affect infrastructure 
                and billing across all customer organizations.
              </p>
            </div>

            <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">
              Platform AI & Infrastructure Configuration
            </h2>
